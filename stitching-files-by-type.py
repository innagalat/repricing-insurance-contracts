...
col_list_res_ls = ['AGE_AT_ENTRY', 'SPCODE', 'ANNUAL_PREM_1','BE_RESERVE', 'L_LIFE_ID',
                'POL_NUMBER','PREMIUM_TYPE','SUM_ASSURED','OAP', 
                'POL_FEE_PP','ENTRY_MONTH', 'ENTRY_YEAR', 'ANNUAL_PREM_25',
                'ANNUAL_PREM_13', 'ANNUAL_PREM_25']

col_list_res_ip = ['AGE_AT_ENTRY', 'SPCODE', 'POL_NUMBER','SUM_INS', 'B_BEN_NO',
                'MOS_PV_PREM', 'BE_RESERVE', 'L_LIFE_ID', 'ANNUAL_PREM_1', 
                'POLICY_FEE','ENTRY_MONTH', 'ENTRY_YEAR', 
                'ANNUAL_PREM_12', 'ANNUAL_PREM_24']
...
col_list_mpf_ip = ['POL_NUMBER','L_LIFE_ID','AGE_AT_ENTRY', 'PREMIUM_TYPE',
                ...
                'BENEFIT_CODE', 'DEFER_PER_MP', 'BEN_PERIOD', 'OCC_CLASS', 
...
columns_to_rename_ip_res = {'SUM_INS':'SUM_ASSURED', 'ANN_PHI_BEN':'SUM_ASSURED',
                            'ANNUAL_PREM':'B_OFF_APREM',
                            'ANNUAL_PREM_01':'ANNUAL_PREM_1',
                            'ANNUAL_PREM_12':'ANNUAL_PREM_13'}

columns_to_rename_ip_mpf = {'SUM_INS':'SUM_ASSURED', 'ANN_PHI_BEN':'SUM_ASSURED',
                            'ANNUAL_PREM':'B_OFF_APREM',
                            'ANNUAL_PREM_12':'ANNUAL_PREM_13',
                            'ANNUAL_PREM_01':'ANNUAL_PREM_1'}
...
def concat_by_type(folderdir, csv_list, col_list, columns_to_rename):
    ...
    print item, (': # rows: '), df.shape[0], ('# columns: '), df.shape[1]
    if size_count != df.shape[0]:
        print ('for '), item, (' check concat-by_type function, # rows is wrong')
    return df
...
if tra_res_csv_list:
    print ('Trauma benefits are included')
...
print('after concat Results files with MPF files:')
print('number of rows RESULTS VS MPF files '), df_death.shape[0], ('='), df_death_mpf.shape[0] 
print('number of rows RESULTS VS MPF files '), df_tpd.shape[0],   ('='), df_tpd_mpf.shape[0] 
print('number of rows RESULTS VS MPF files '), df_ip.shape[0],    ('='), df_ip_mpf.shape[0]
if tra_res_csv_list:
    print('number of rows RESULTS VS MPF files '), df_tra.shape[0],('='), df_tra_mpf.shape[0]  
...
print ('Checking stage 1. Checking length of files before concat RES and MPF')
...
print ('Checking stage 2. Checking length of files length after concat')
